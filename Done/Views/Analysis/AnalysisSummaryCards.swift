...
struct AnalysisSummaryCards: View {
    ...
    var body: some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
            SummaryCard(title: "Scheduled", value: String(format: "%.1fh", hours), icon: "clock", color: .blue)
            SummaryCard(title: "Completed", value: "\(completed)", icon: "checkmark.circle", color: .green)
